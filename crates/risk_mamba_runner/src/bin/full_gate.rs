...
#[derive(Parser, Debug)]
#[command(about = "Full stateless correctness gate (head/block/full)")]
struct Args {
    #[arg(long, alias = "bundle", default_value = "mamba/static_bundle_full_v1_mainline")]
    bundle_dir: String,
    #[arg(long)]
    golden_min: Option<String>,
    #[arg(long)]
    golden_dbg: Option<String>,
    #[arg(long, default_value = "both", value_parser = ["min", "dbg", "both"])]
    mode: String,
    #[arg(long, default_value = "all", value_parser = ["head", "block", "full", "all"])]
    stage: String,
...
impl GateSummary {
    fn record(&mut self, key: &str, tier: &str, max_abs: f32, max_rel: f32, status: &'static str) {
        let id = format!("{}::{}", tier, key);
        let entry = self.entries.entry(id).or_insert(GateEntry {
            key: key.to_string(),
            tier: tier.to_string(),
            max_abs,
            max_rel,
        ...
        if max_abs > entry.max_abs {
            entry.max_abs = max_abs;
        }
        if max_rel > entry.max_rel {
    ...
    fn print(&self) {
        ...
        for entry in entries {
            println!(
                "  {} tier={} max_abs={} max_rel={} status={}",
                entry.key, entry.tier, entry.max_abs, entry.max_rel, entry.status
            );
        }
    }
}

fn tier_opts(policy: &GatePolicyRuntime, name: &str) -> Result<DiffOptions, String> {
    policy
        .tiers
        .get(name)
        .copied()
        .ok_or_else(|| format!("missing tier {}", name))
}
...
fn diff_opts_for_key(
    ...
    for pat in &policy.patterns {
        if pat.regex.is_match(name) {
            if dbg_strict
                && pat.tier == "dbg-activation"
                && (name.contains("/residual_in") || name.contains("/residual_out"))
            {
                continue;
            }
            tier = pat.tier.clone();
    ...
    let opts = policy.tiers.get(&tier).copied().unwrap_or_else(|| {
        policy
            .tiers
            .get(default_tier)
            .copied()
            .unwrap_or(DiffOptions {
                atol: 1e-5,
                rtol: 1e-4,
                fail_fast: false,
                dump_first: false,
...
fn load_gate_policy(path: &Path, base: DiffOptions) -> Result<GatePolicyRuntime, String> {
    ...
    for pat in policy.patterns {
        let regex = Regex::new(&pat.regex).map_err(|e| e.to_string())?;
...
fn activation_stats(values: &[f32]) -> StatSummary {
    let mut sum = 0.0f64;
    let mut sum_sq = 0.0f64;
    let mut max_abs = 0.0f32;
    let mut abs_vals = Vec::with_capacity(values.len());
    for &v in values {
        let a = v.abs();
        if a > max_abs {
            max_abs = a;
    ...
    let p99_abs = if abs_vals.is_empty() {
        0.0
    } else {
        let idx = ((abs_vals.len() as f64 - 1.0) * 0.99).round() as usize;
        abs_vals.select_nth_unstable_by(idx, |a, b| a.partial_cmp(b).unwrap());
        abs_vals[idx]
    };
    StatSummary {
        mean,
...
fn check_activation_stats(name: &str, actual: &[f32], expected: &[f32]) -> Result<(), String> {
    let a = activation_stats(actual);
    let e = activation_stats(expected);
    println!(
        "STAT {} actual(mean={} std={} max_abs={} p99_abs={}) expected(mean={} std={} max_abs={} p99_abs={})",
        name,
        a.mean,
        a.std,
        a.max_abs,
    ...
    if e.std > 0.0 {
        let ratio = a.std / e.std;
        if ratio > 5.0 || ratio < 0.2 {
            return Err(format!("activation std ratio out of range for {}", name));
        }
    }
    if e.max_abs > 0.0 {
        let ratio = a.max_abs / e.max_abs;
        if ratio > 10.0 || ratio < 0.1 {
            return Err(format!("activation max_abs ratio out of range for {}", name));
        }
    }
    if e.p99_abs > 0.0 {
        let ratio = a.p99_abs / e.p99_abs;
        if ratio > 10.0 || ratio < 0.1 {
            return Err(format!("activation p99_abs ratio out of range for {}", name));
        }
    }
    Ok(())
}
...
fn compare_f32(
    ...
    if has_mismatch {
        ...
        println!(
            "FAIL {} max_abs={} max_rel={} idx={} shape={:?} tier={} atol={} rtol={}",
            name,
            max_abs,
            max_rel,
        ...
        summary.record(name, tier, max_abs, max_rel, "FAIL");
        return Err(format!(
            "{} mismatch max_abs={} idx={} shape={:?}",
            name,
            max_abs,
            format_index(max_idx, shape),
            shape
    ...
    if tier == "dbg-activation" {
        check_activation_stats(name, actual, expected)?;
    }
    println!(
        "PASS {} max_abs={} max_rel={} shape={:?} tier={} atol={} rtol={}",
        name,
        max_abs,
        max_rel,
        shape,
    ...
    summary.record(name, tier, max_abs, max_rel, "PASS");
    Ok(())
}
...
struct GoldenDbg {
    ...
    orig_batch: usize,
    summary_ptr: *mut GateSummary,
    policy_ptr: *const GatePolicyRuntime,
    dbg_strict: bool,
    layer_buffers: HashMap<String, LayerBuffers>,
}
...
#[derive(Default)]
struct LayerBuffers {
    residual_in: Option<Vec<f32>>,
    mamba_out: Option<Vec<f32>>,
    mlp_out: Option<Vec<f32>>,
}
...
impl DbgTaps for GoldenDbg {
    fn tap_f32(&mut self, key: &str, shape: &[usize], data: &[f32]) -> Result<(), KernelError> {
        ...
        let policy = unsafe { &*self.policy_ptr };
        let (opts, tier) = diff_opts_for_key(key, policy, self.dbg_strict, "dbg-critical");
        let summary = unsafe { &mut *self.summary_ptr };
        ...
        if arr.shape == shape {
            if let Some(indices) = sample_indices {
                ...
                for &idx in indices {
                    ...
                    compare_f32(&sample_key, actual_sample, expected_sample, &sample_shape, opts, tier.as_str(), summary)
                        .map_err(|e| KernelError::DebugTapError(e))?;
                }
            } else {
                compare_f32(key, data, &expected, shape, opts, tier.as_str(), summary)
                    .map_err(|e| KernelError::DebugTapError(e))?;
            }
        } else if let Some(sample) = self.sample {
            if arr.shape.len() == shape.len() + 1
                && !arr.shape.is_empty()
                ...
                && &arr.shape[1..] == shape
            {
                ...
                let end = start + stride;
                let expected_sample = &expected[start..end];
                compare_f32(key, data, expected_sample, shape, opts, tier.as_str(), summary)
                    .map_err(|e| KernelError::DebugTapError(e))?;
            } else if arr.shape.len() == shape.len()
                && !arr.shape.is_empty()
                && !shape.is_empty()
                && arr.shape[0] == self.orig_batch
                && shape[0] == 1
        ...
        if let Some((layer, tap)) = key.split_once('/') {
            if layer.starts_with("layer") {
                let entry = self
                    ...
                    .or_default();
                match tap {
                    "residual_in" => entry.residual_in = Some(data.to_vec()),
                    "mamba_out" => entry.mamba_out = Some(data.to_vec()),
                    "mlp_out" => entry.mlp_out = Some(data.to_vec()),
                    "residual_out" => {
                        if let (Some(res_in), Some(mamba), Some(mlp)) = (
                            entry.residual_in.as_ref(),
                            entry.mamba_out.as_ref(),
                            entry.mlp_out.as_ref(),
                        ) {
                            if res_in.len() != data.len()
                                || mamba.len() != data.len()
                                || mlp.len() != data.len()
                            {
                                return Err(KernelError::DebugTapError(format!(
                            ...
                            for i in 0..data.len() {
                                let closure = data[i] - res_in[i] - mamba[i] - mlp[i];
                                let a = closure.abs();
                                if a > max_abs {
                                    max_abs = a;
                                }
                            ...
                            let tol = (scale * 1e-7).max(1e-2);
                            if max_abs > tol {
                                return Err(KernelError::DebugTapError(format!(
                                    "closure mismatch {} max_abs={} tol={}",
                                    layer, max_abs, tol
                                )));
                            }
                        }
...
fn main() -> Result<(), String> {
    ...
    let summary_ptr = &mut summary as *mut GateSummary;
    let bundle_dir = PathBuf::from(args.bundle_dir);
    let base_opts = DiffOptions {
        atol: args.atol,
        rtol: args.rtol,
    ...
    let policy_path = {
        let p = PathBuf::from(&args.gate_policy);
        if p.is_relative() {
    ...
    let policy = load_gate_policy(&policy_path, base_opts)?;
    println!(
    ...
    for pat in &policy.patterns {
        println!("  {} -> {}", pat.regex_str, pat.tier);
    }
    println!("dbg_strict={}", args.dbg_strict);
    let policy_ptr = &policy as *const GatePolicyRuntime;
    ...
    let run_min = args.mode == "min" || args.mode == "both";
    let run_dbg = args.mode == "dbg" || args.mode == "both";
    let run_head = args.stage == "head" || args.stage == "all";
    let run_block = args.stage == "block" || args.stage == "all";
    let run_full = args.stage == "full" || args.stage == "all";

    if args.stop_after.is_some() && !run_dbg {
        return Err("stop_after only valid in dbg mode".to_string());
    }
    if args.dbg_sample.is_some() && !run_dbg {
    ...
    let min_opts = tier_opts(&policy, "min")?;
    let dbg_critical_opts = tier_opts(&policy, "dbg-critical")?;

    let min_npz = if let Some(path) = args.golden_min.as_ref() {
    ...
    let dbg_npz = if let Some(path) = args.golden_dbg.as_ref() {
        let p = PathBuf::from(path);
        if p.is_relative() {
            bundle_dir.join(p)
        } else {
        ...
        bundle_dir.join("golden_full_dbg_v1_mainline.npz")
    };

    let min = if run_min { Some(load_npz(&min_npz)?) } else { None };
    let dbg = if run_dbg { Some(load_npz(&dbg_npz)?) } else { None };

    let mut run_head_only = |data_npz: &HashMap<String, NpyArray>| -> Result<(), String> {
        let summary = unsafe { &mut *summary_ptr };
        let head_in_arr = data_npz
            .get("head_in")
            .ok_or_else(|| "missing head_in".to_string())?;
        let head_in = head_in_arr.as_f32()?;
        let head_shape = head_in_arr.shape.clone();
        let (batch, dim) = match head_shape.as_slice() {
            [b, d] => (*b, *d),
            [d] => (1, *d),
            _ => return Err(format!("head_in shape unsupported {:?}", head_shape)),
        };
        if dim != cfg.d_model {
            return Err("head_in dim mismatch".to_string());
        }
        ...
        let mut logits_out = vec![0.0f32; batch * cfg.n_class];
        for b in 0..batch {
            let head_row = &head_in[b * cfg.d_model..][..cfg.d_model];
            for c in 0..cfg.n_class {
                let w_row = &weights.head_w[c * cfg.d_model..][..cfg.d_model];
                let mut acc = weights.head_b[c];
                for i in 0..cfg.d_model {
                    acc += w_row[i] * head_row[i];
                }
                logits_out[b * cfg.n_class + c] = acc;
            }
        }
        let mut delta_out = vec![0.0f32; batch];
        if cfg.n_class >= 2 {
            for b in 0..batch {
                let base = b * cfg.n_class;
                delta_out[b] = logits_out[base + 1] - logits_out[base];
            }
        } else {
            for b in 0..batch {
                delta_out[b] = logits_out[b * cfg.n_class];
            }
        }
        ...
        println!("OK: head_only");
        Ok(())
    };

    let mut run_once = |data_npz: &HashMap<String, NpyArray>,
                        compare_taps: bool,
                        compare_logits: bool,
                        check_near_thr: bool,
                        label: &str|
     -> Result<(), String> {
        ...
        if compare_logits && logits_exp_full.is_none() && margin_exp_full.is_none() {
            return Err("missing logits or delta_margin/margin".to_string());
        }
        if compare_logits && margin_key == Some("margin") {
            if let (Some(logits), Some(margin)) =
                (logits_exp_full.as_ref(), margin_exp_full.as_ref())
            {
        ...
        let dbg_sample_indices = if let Some(count) = args.dbg_sample {
            if count == 0 {
                ...
                let mut indices: Vec<usize> =
                    index::sample(&mut rng, orig_batch, count).into_vec();
        ...
        let mut pred_margin_full = if full_run && check_near_thr {
        ...
        for sample in sample_indices {
            ...
            let mut dbg_sink = if compare_taps {
                Some(GoldenDbg::new(
                    dbg.clone().ok_or_else(|| "missing dbg npz".to_string())?,
            ...
            let dbg_ref: Option<&mut dyn DbgTaps> =
                dbg_sink.as_mut().map(|s| s as &mut dyn DbgTaps);
    ...
    if run_min {
        let min_npz_ref = min.as_ref().ok_or_else(|| "missing min npz".to_string())?;
        run_once(min_npz_ref, false, true, true, "min")?;
    }
    if run_dbg {
        let dbg_npz_ref = dbg.as_ref().ok_or_else(|| "missing dbg npz".to_string())?;
        if run_head {
            run_head_only(dbg_npz_ref)?;
        }
        if run_block {
            run_once(dbg_npz_ref, true, false, false, "dbg-block")?;
        }
        if run_full {
            run_once(dbg_npz_ref, true, true, true, "dbg-full")?;
        }
    }
